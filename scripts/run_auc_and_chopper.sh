...
if [[ ! -e scripts/chopper.py ]]; then
	echo "I can't find scripts/chopper.py."
	exit
fi

if [[ ! -e scripts/auc_chasman.jar ]]; then
...
for f in $1/*.list; do
	...
	java -jar scripts/auc_chasman.jar -t list -o ${pathbase} ${f} > ${pathbase}.auc
	...
	python scripts/chopper.py ${pathbase}.opr ${pathbase}.opr ${pathbase}.pr
done
